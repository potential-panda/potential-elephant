# Spec: Yahoo Japan Finance BBS Scraper (Phase 1)

## 1. Goal

Build a robust, headless scraping pipeline to collect
1. up to 200 recent comments per ticker daily
2. the evaluation per ticker daily

Ensuring data is stored in a queryable, time-indexed format.

## 2. Scraping Logic

### 2.1 URL Structure

The target for each stock is the "All Comments" history page:
`https://finance.yahoo.co.jp/quote/{ticker}.T/forum`
*(Note: Use `.T` for Tokyo Stock Exchange, e.g., `7203.T` for Toyota).*

### 2.2 Extraction Specification

#### 2.2.1 Evaluation

The container of the evaluation section:
